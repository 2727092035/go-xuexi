...
2. 对照 demo 注释理解每段语法：为什么需要它、核心写法、运行结果说明什么。
3. 再读当天 `notes.md`，重点看语法说明、代码逐段详解、常见坑、和 JS/TS 的差异。
4. 完成 `exercise/main.go` 的基础题，确保 `go run phase1-basics/dayXX/exercise/main.go` 能跑通。
5. 完成 1 到 2 个加餐题，强迫自己把知识组合起来用。
6. 最后用 3 到 5 句话复盘：今天学了什么、卡在哪里、明天要补什么。

## 后续 demo 和笔记要求

从 Day 6 开始，后续每天的 demo 要更偏教学型：

- demo 代码按主题分段，不把所有内容混在一个大函数里。
- 每段代码前用中文注释解释语法点，例如 `error` 是什么、`strings.Split` 返回什么、`strconv.Atoi` 为什么会返回两个值。
- 只要 demo 里出现新语法、新标准库函数、新方法调用或新写法，就必须在旁边讲清楚“来源、用途、参数、返回值、为什么这里要用”。不能让初学者自己猜 `strings.TrimSpace(raw)`、`strconv.Atoi(text)` 这种调用是什么意思。
- 每天 notes 必须增加“demo 代码逐段详解”，用文字说明每段示例对应哪个语法点。
- 每个语法点都要配一个最小例子，再配一个接近业务的小例子。
- 练习题要从基础到组合递进，基础题练语法，组合题练真实使用。
...
- 会检查错误并提前返回。
- 会把字符串转数字，并处理转换失败。
- 能读懂 `strings.Split`、`strings.TrimSpace`、`strconv.Atoi` 的输入输出。
- 能解释函数为什么要返回 `(value, error)`。

demo 教学重点：
- `error` 类型是什么，`nil` 代表什么。
- `errors.New` 和 `fmt.Errorf` 分别适合什么场景。
- `strconv.Atoi` 为什么必须检查错误。
- `strings.Split` 拆分字符串后如何遍历。
- 连续校验时为什么常用提前返回。
...
demo 教学重点：
- 如何从一行字符串逐步解析出结构体。
- 解析、校验、统计、输出为什么要拆成不同函数。
- `[]Student`、`map[string]int`、`error` 如何组合使用。
- `main` 应该负责组织流程，而不是堆业务逻辑。
...
demo 教学重点：
- 一个目录和一个 package 的关系。
- `go run file.go` 和 `go run .` 的区别。
- 大写导出、小写不导出的规则。
- 文件拆分不等于 package 拆分，同目录下多个文件仍属于同一个 package。
...
demo 教学重点：
- `_test.go` 文件为什么不会进入普通编译。
- `func TestXxx(t *testing.T)` 的命名规则。
- 表格驱动测试的结构：name、input、want、wantErr。
- 如何用边界用例暴露 bug。
...
- 重构前后代码职责怎么变化。
- 如何判断一个函数是否做了太多事。
- goroutine/channel/context 的最小可理解示例。
- 并发练习只做入门，重点是理解数据如何流动和如何收尾。
